...
public class ProductScreen extends MenuBar {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        System.recommendations.insert(new Edge(new User(SignIn.whoami),newProduct.getType()));

        //girilen kategorinin agirligini 1 azaltiyorum firebase den.
        if(newProduct.getType().equals("ELEKTRONIK")){
            mDatabase.child("Graph").child(String.valueOf(SignIn.whoami.hashCode())).child("elekW").
                    setValue(System.recommendations.getEdge(new User(SignIn.whoami),newProduct.getType()).getWeight());
        }
        else if(newProduct.getType().equals("DENEY MALZEMELERI")){
            mDatabase.child("Graph").child(String.valueOf(SignIn.whoami.hashCode())).child("deneyW").
                    setValue(System.recommendations.getEdge(new User(SignIn.whoami),newProduct.getType()).getWeight());
        }
        else if(newProduct.getType().equals("KITAPLAR")){
            mDatabase.child("Graph").child(String.valueOf(SignIn.whoami.hashCode())).child("kitapW").
                    setValue(System.recommendations.getEdge(new User(SignIn.whoami),newProduct.getType()).getWeight());
        }
        else if(newProduct.getType().equals("EV EŞYALARI")){
            mDatabase.child("Graph").child(String.valueOf(SignIn.whoami.hashCode())).child("esyaW").
                    setValue(System.recommendations.getEdge(new User(SignIn.whoami),newProduct.getType()).getWeight());
        }
        else if(newProduct.getType().equals("ETKINLIK-BILET")){
            mDatabase.child("Graph").child(String.valueOf(SignIn.whoami.hashCode())).child("biletW").
                    setValue(System.recommendations.getEdge(new User(SignIn.whoami),newProduct.getType()).getWeight());
        }
        ...
        tf = Typeface.createFromAsset(getAssets(), "opensanss.ttf");
